...
const fieldPatterns: Array<{
  ...
  // Email patterns
  { pattern: /^(email|e_mail|mail|email_address)$/i, type: 'email', confidence: 0.9 },
  { pattern: /email$/i, type: 'email', confidence: 0.8 },
  
  // Name patterns
  { pattern: /^(first_name|firstName|first|fname|given_name)$/i, type: 'firstName', confidence: 0.9 },
  { pattern: /^(last_name|lastName|last|lname|surname|family_name)$/i, type: 'lastName', confidence: 0.9 },
  { pattern: /^(full_name|fullName|name|display_name)$/i, type: 'fullName', confidence: 0.8 },
  { pattern: /name$/i, type: 'fullName', confidence: 0.6 },
  
  // Contact patterns
  { pattern: /^(phone|telephone|tel|phone_number|mobile|cell)$/i, type: 'phone', confidence: 0.9 },
  { pattern: /(phone|tel)$/i, type: 'phone', confidence: 0.7 },
  
  // Address patterns
  { pattern: /^(address|street|street_address|addr)$/i, type: 'address', confidence: 0.9 },
  { pattern: /^(city|town)$/i, type: 'city', confidence: 0.9 },
  { pattern: /^(country|nation)$/i, type: 'country', confidence: 0.9 },
  { pattern: /^(zip|zipcode|zip_code|postal|postal_code|postcode)$/i, type: 'zipCode', confidence: 0.9 },
  
  // Company patterns
  { pattern: /^(company|corp|corporation|business|organization|org)$/i, type: 'company', confidence: 0.9 },
  { pattern: /^(job|position|role|title|job_title|occupation)$/i, type: 'jobTitle', confidence: 0.8 },
  
  // Date patterns
  { pattern: /^(date|created|updated|birth|birthday|born|timestamp)$/i, type: 'date', confidence: 0.8 },
  { pattern: /(date|time)$/i, type: 'date', confidence: 0.6 },
  
  // Web patterns
  { pattern: /^(url|link|website|site|homepage)$/i, type: 'url', confidence: 0.9 },
  { pattern: /^(username|user|login|handle)$/i, type: 'username', confidence: 0.8 },
  { pattern: /^(password|pass|pwd|secret)$/i, type: 'password', confidence: 0.9 },
  { pattern: /^(id|uuid|guid|identifier)$/i, type: 'uuid', confidence: 0.8 },
  { pattern: /^(avatar|profile_pic|picture|photo|image)$/i, type: 'avatar', confidence: 0.8 },
  
  // Financial patterns
  { pattern: /^(price|cost|amount|value|fee)$/i, type: 'price', confidence: 0.8 },
  { pattern: /^(currency|curr)$/i, type: 'currency', confidence: 0.9 },
  { pattern: /^(credit_card|card|cc|card_number)$/i, type: 'creditCard', confidence: 0.9 },
  ...
  { pattern: /^(description|desc|summary|bio|about)$/i, type: 'description', confidence: 0.7 },
  { pattern: /^(title|heading|subject)$/i, type: 'title', confidence: 0.6 },
  { pattern: /^(image|img|pic|picture|photo)$/i, type: 'image', confidence: 0.7 },
];
