...
const categoryColors = {
  ...
  ecommerce: 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300',
  ...
  business: 'bg-slate-100 dark:bg-slate-900/30 text-slate-700 dark:text-slate-300',
} as const;

export function JSONTemplates({ onTemplateSelectAction, className = "" }: JSONTemplatesProps) {
  const [isExpanded, setIsExpanded] = useState(false);
  const [searchQuery, setSearchQuery] = useState("");
  const [selectedCategory, setSelectedCategory] = useState<JSONTemplate['category'] | 'all'>('all');
  ...
  const categories = ['all', 'user', 'ecommerce', 'blog', 'api', 'business'] as const;
            ...
            <div className="bg-white/70 dark:bg-slate-900/70 rounded-lg border border-slate-200 dark:border-slate-700 p-4 space-y-4">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                {categories.map((category) => {
                  const Icon = category === 'all' ? FileCode : categoryIcons[category as keyof typeof categoryIcons];
                        ...
                        >
                          ...
                          >
                            ...
                            <div className="flex items-center gap-2 w-full">
                              ...
                              <Badge 
                                variant="secondary" 
                                className={`text-xs ${categoryColors[template.category]} ml-auto`}
                              >
